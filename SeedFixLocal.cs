...
optionsBuilder.UseSqlServer("Server=.\\SQLEXPRESS;Database=PMS_DB;Trusted_Connection=True;TrustServerCertificate=True;MultipleActiveResultSets=true");

using (var db = new ApplicationDbContext(optionsBuilder.Options))
{
    Console.WriteLine("--- Checking Stage 3 Accounts against Local DB ---");
    ...
    try
    {
        ...
        if (missing.Contains(214))
            db.Accounts.Add(new Account { Id = 214, Code = "214", NameAr = "?????? ?????? ?????", NameEn = "Commission-Related Payables", Type = PMS.Domain.Enums.BackOffice.AccountType.Liability, ParentAccountId = 21, IsGroup = true,
 ...
 CurrentBalance = 0m, IsActive = true, CreatedAt = now, CreatedBy = "System" });
       ...
       if (missing.Contains(5141))
        ...
        Console.WriteLine("SUCCESS: Missing accounts inserted into Local DB.");
    }
    catch (Exception ex)
    {
        tx.Rollback();
